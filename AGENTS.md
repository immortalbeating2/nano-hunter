...
- 项目级 agent 角色池保存在官方加载路径 `.codex/agents/`，并通过 `.codex/config.toml` 的 `[agents]` 注册
- 当前 `max_depth = 1`；如 `multi_agent_v2` 与显式 `max_threads` 存在兼容性冲突，允许暂时注释 `max_threads` 并使用 Codex 官方默认并发上限，但项目执行层仍按“单轮通常启用 `2-4` 个最相关角色”约束
