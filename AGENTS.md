...
- 面向用户的说明、过程更新、提权理由、任务清单弹窗、子代理任务描述和项目文档默认使用中文
- 提交信息继续使用“中文 + English”
...
- 项目级 agent 角色池保存在官方加载路径 `.codex/agents/`，并通过项目级 `.codex/config.toml` 的 `[agents.<role>]` 注册角色
- `max_depth`、`max_threads` 等通用 agent 行为默认放在用户级全局 `~/.codex/config.toml`；项目仓库不强制提交这些机器级偏好
- 当前全局建议为 `max_depth = 1`；如 `multi_agent_v2` 与显式 `max_threads` 存在兼容性冲突，允许暂时注释 `max_threads` 并使用 Codex 官方默认并发上限，但项目执行层仍按“单轮通常启用 `2-4` 个最相关角色”约束
...
- 项目内新增或修改的代码注释统一使用中文
- 注释目标从“少量解释复杂处”提升为“达到文档可阅读性”：后续接手者应能通过文件头、分段注释和关键流程注释，看懂每一段重要代码负责什么、为什么存在、和谁协作
- 已开发阶段的核心脚本在后续触达时必须顺手补注释；将来阶段新增核心脚本必须在首次提交时达到本节要求，不把注释补全推迟到阶段收口
- 涉及玩家控制、战斗判定、能力门控、场景切换、房间推进、checkpoint、HUD 快照翻译、敌人行为循环、资产接入、运行态复核和插件 / MCP 工具链等关键流程时，必须补充能帮助后续快速理解的引导性注释
- 非平凡运行时脚本默认需要文件头职责注释，至少写明“这个脚本负责什么、不负责什么、主要和谁协作”
- 单文件一旦超过约 `80` 行、或同一文件承担多个职责，应使用分段注释把主要逻辑块隔开，避免读者只能靠来回跳函数推断结构
- 每个重要逻辑段落都应有短注释说明本段意图；复杂条件、状态切换、信号收发、测试 fixture、房间链路和配置读取必须说明“为什么这样做”
- 阶段测试文件默认需要文件头说明，明确“该文件保护哪个阶段的什么能力”，并在测试主体与辅助函数之间做分段注释
- 关键测试 helper 默认需要说明它构造了什么场景、保护什么回归、为什么不能直接复用运行时入口
...
- 若某次阶段开发触达超过 `3` 个核心脚本，应把“注释可读性补强”列入该阶段收口检查；如果暂时不能补完，必须在当日日志写明剩余文件和原因
