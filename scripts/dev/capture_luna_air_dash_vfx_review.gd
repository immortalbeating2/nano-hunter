...
# 捕获 Luna Air Dash 正式替换批次的运行态 VFX 复核截图。
# 输出默认进入 tests/artifacts/local/，作为本地证据，不进入普通提交。

const PLAYER_SCENE_PATH := "res://scenes/player/player_placeholder.tscn"
const OUT_DIR := "res://tests/artifacts/local/animation-runtime-replacement/arp_16_luna_air_dash_trail_vfx"
const OUT_IMAGE := "%s/luna_air_dash_trail_runtime.png" % OUT_DIR
const OUT_REPORT := "%s/luna_air_dash_trail_runtime_report.json" % OUT_DIR
const VIEWPORT_SIZE := Vector2i(960, 540)
const TARGET_BODY_ASSET_ID := "luna_air_dash_body_runtime_sheet_ai03"
const TARGET_BODY_SPRITEFRAMES := "res://assets/art/characters/player/sprite_sheets/runtime_replacement/luna_air_dash_body_runtime_sheet_ai03.spriteframes.tres"
const TARGET_TRAIL_ASSET_ID := "stage14_air_dash_trail_ai01"
const TARGET_TRAIL_TEXTURE := "res://assets/art/vfx/stage14_air_dash_trail_ai01.png"
const SAMPLE_STEP := 6
...
# 主入口：构造玩家 dash 状态，保存截图和结构化报告。
func _run() -> void:
	var result := await _capture_air_dash_vfx()
	quit(result)


# 实例化玩家、构造地面 dash，并截取运行态画面。
func _capture_air_dash_vfx() -> int:
	...
	if save_error != OK:
		push_error("Failed to save Luna Air Dash runtime screenshot: %s" % save_error)
		world.free()
		return 1
	...
	if not bool(report.get("ok", false)):
		push_error("Luna Air Dash runtime review failed: %s" % report)
		world.free()
		return 1

	print("Luna Air Dash runtime review capture OK: %s" % OUT_IMAGE)
	print("Luna Air Dash runtime review report: %s" % OUT_REPORT)
	world.free()
	return 0


# 等待玩家稳定落地，保证 dash 从地面状态起手。
...
# 汇总截图采样、玩家状态、body runtime 和 trail VFX 约束。
func _build_report(image: Image, player: CharacterBody2D) -> Dictionary:
	...
	var trail_visual := player.get_node_or_null("AirDashTrailArt") as Sprite2D
	...
	var trail_report := _inspect_trail_visual(trail_visual)
	var has_collision_child := _has_collision_or_area_child(trail_visual)
	...
	var ok := (
		...
		and bool(trail_report.get("visible", false))
		and bool(trail_report.get("resource_ok", false))
		and bool(trail_report.get("metadata_ok", false))
	...
	return {
		"ok": ok,
		"review_id": "arp_16_luna_air_dash_trail_vfx_runtime",
		"scene": PLAYER_SCENE_PATH,
		"image": OUT_IMAGE,
		"viewport_size": [VIEWPORT_SIZE.x, VIEWPORT_SIZE.y],
		"player_state": state,
		"runtime_visual": runtime_report,
		"air_dash_trail_visual": trail_report,
		"air_dash_trail_has_collision_or_area_child": has_collision_child,
		"image_stats": image_stats,
		"manual_review_guidance": "Open the screenshot and confirm trail direction, overlap with Luna body, alpha, and readability.",
		"boundary": "Runtime screenshot and metadata review only; dash movement, collision and ability availability remain authored by player gameplay code.",
	}


# 检查 Luna runtime visual 是否处于 clean Air Dash body。
func _inspect_runtime_visual(visual: AnimatedSprite2D) -> Dictionary:
	...
	var resource_ok := (
		...
		and visual.animation == &"air_dash_body"
	)
	return {
		"exists": true,
...
# 检查 Air Dash trail VFX 的资源、朝向和 no-collision / no-damage metadata。
func _inspect_trail_visual(visual: Sprite2D) -> Dictionary:
	if visual == null:
		return {"exists": false, "resource_ok": false, "metadata_ok": false}
	var resource_path := visual.texture.resource_path if visual.texture != null else ""
	...
	var resource_ok := str(visual.get_meta("asset_id", "")) == TARGET_TRAIL_ASSET_ID and resource_path == TARGET_TRAIL_TEXTURE
