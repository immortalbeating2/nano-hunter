...
- Godot 插件扫描端口：`6505-6534`
- Node server 会写入 lock/heartbeat：`%LOCALAPPDATA%/godot-mcp-pro/bridges/<port>.json`
- 新 MCP tool：`get_bridge_status`
- Godot 插件连接后会发送 `godot_hello`，Node bridge 只接受 workspace 匹配的 Godot editor
