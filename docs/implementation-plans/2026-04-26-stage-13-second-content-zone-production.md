...
将 Stage 13 从正式文档设计推进到可实施入口。当前文件是后续实现用执行清单，固定 `瘴泽妖域 + 10 主线房 + 2 支路` 的范围，不在 preflight 中混入玩法实现。
...
- [x] 更新 `docs/assets/asset-manifest.md`，追加 `biome_02_miasma_marsh` 与 Stage 13 第一批资产需求
...
- [x] 新增 `MiasmaCasterEnemy` 行为脚本、场景和配置资源
- [x] 新增腐瘴危险节点或房间侧最小危险逻辑
- [x] 覆盖瘴气妖术投射者、腐瘴危险与 checkpoint 互动的 GUT
- [x] 保持现有玩家受击、无敌和恢复契约不回归

### C. 封印门控与两条支路

- [x] 新增封印门控的最小节点、脚本或房间侧逻辑
- [x] 新增资源 / 恢复收益支路
- [x] 新增风险挑战收益支路
...
- [x] 补 Stage 13 HUD 区域目标、腐瘴危险、封印门控和支路反馈文案
...
- `10` 个主线房间、`2` 条小支路、瘴气妖术投射者、腐瘴危险、封印门控和区域终点均有验证证据
...
- 已完成：Stage 13 主线入口、10 个主线房间、2 条小支路、瘴气妖术投射者、腐瘴危险、封印门控、区域终点房、Stage 13 灰盒主路径自动化与轻量占位资产。
- 自动化覆盖：`tests/stage13/test_stage_13_second_content_zone_production.gd` 覆盖入口、主线顺序、支路返回、新敌人配置、瘴气伤害、封印门控、资产 manifest 与 `Main.tscn` 灰盒推进。
- 排障记录：补充 SVG 资产后，首轮全量 GUT 因新 SVG 未生成 `.import` 导致 `miasma_caster_enemy.tscn` 无法按 Texture2D 加载；运行 `godot --headless --path . --import` 后生成导入文件并复测通过。
...
- 覆盖项：从 `Main.tscn` 到 Stage13 入口、完整 Stage13 主路径、两条支路、瘴气反馈、checkpoint 恢复、封印门关闭到开启。
