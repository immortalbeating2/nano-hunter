...
- [ ] 先补红测，暴露当前玩家没有 `空中攻击`
- [ ] 新增最小 `空中攻击` 状态、判定与恢复逻辑
- [ ] 明确与跳跃、落地、地面普通攻击、受击的互斥关系
- [ ] 只做最小成立版本，不引入连段、浮空追打或第二个新动作
...
- [ ] 先补红测，固定当前仅有两类敌人
- [ ] 新增第 `3` 类普通敌人，继续复用现有敌人模板与配置资源入口
- [ ] 设计 `2-3` 组混合遭遇，确保其节奏与现有两类敌人明显不同
- [ ] 至少有 `1` 个房间或遭遇节点明确体现 `空中攻击` 的价值
...
- [ ] 先补红测，固定当前区域没有本轮成长反馈
- [ ] 新增恢复点激活、收集物计数或状态快照
- [ ] 将其绑定到区域推进，而不是扩成正式经济 / 存档系统
- [ ] 让支路与挑战房的收益通过恢复点或收集物被玩家明确感知
...
- [ ] 在 stage9 主区域基线上新增 `1` 条可选支路
- [ ] 新增 `1` 个挑战房
- [ ] 保证主线推进仍稳定成立，不因支路扩展打断主链路
- [ ] 不新开第二个大区域，不扩成半开放地图
...
- [ ] 覆盖 `空中攻击`、第 `3` 类敌人、支路、挑战房、恢复点与收集物快照
- [ ] 确认 Stage 1-9 测试不回归
- [ ] 完成 fresh 自动化验证：
  - `godot --headless --path . --import`
  - Stage 1-10 GUT
...
- [ ] `stage10` 已在 `stage9` 基线上形成更完整的内容型里程碑
- [ ] 玩家新增 `空中攻击` 并有明确玩法价值
- [ ] 第 `3` 类普通敌人已接入，且遭遇节奏明显扩展
- [ ] 区域内存在 `1` 条可选支路与 `1` 个挑战房
- [ ] `恢复点 + 收集物` 的轻量成长反馈闭环已成立
- [ ] Stage 1-10 自动化验证通过
- [ ] 已完成最终人工复核并写回进度文档
