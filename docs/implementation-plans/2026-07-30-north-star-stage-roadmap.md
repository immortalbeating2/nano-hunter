...
- [x] 保留 2 槽上限。
- [x] 将 `marsh_relic` / `warden_sigil` 纳入正式装备选择。
- [x] 新增 2-3 个服务元素序列的圣物或组件。
- [x] 验证装备组合对序列窗口、姿态冷却、恢复或攻击读值有可测影响。

## Stage25 第二区域原型

- [x] 锁定雷泽荒原或等价第二区域主题。
- [x] 新增 `6-8` 房小型区域，不扩到完整商业区域。
- [x] 至少包含一个新区域机制、一个旧区回访口和一个小回环。
- [x] 更新世界图 JSON、发现式地图、房间矩阵和对应测试。
