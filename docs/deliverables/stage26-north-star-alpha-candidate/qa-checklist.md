...
- 外部人工：真人路线理解、操作手感与实体手柄硬件；前两类证据不能替代本类。
...
- [ ] 实体手柄完成基础移动、战斗、Q/E 对应 LB/RB、暂停与 UI 导航复核。
- [ ] 记录手柄型号、系统、漂移 / 死区和按键舒适度结论。

当前状态：自动化与代理运行态已收口；本机 `Input.get_connected_joypads()` 返回空数组。真人和实体手柄保持外部人工门禁，未取得证据前不得勾选。
